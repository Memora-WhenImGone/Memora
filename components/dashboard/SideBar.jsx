...
const SideBar = () => {
  ...
  return (
    <div
      className={`flex flex-col bg-white h-screen border-r 
    border-gray-200 font-normal transition-all duration-300 ${collapsed ? "w-20" : "w-64"}`}
    >
      <div
        className={`top flex flex-row items-center 
        justify-between ${collapsed ? "px-3" : "px-6"} py-6 border-b border-gray-200`}
      >
        <div
          className={`flex flex-row items-center ${collapsed ? "justify-center flex-1" : "gap-3"}`}
        >
          <div className="bg-slate-900 p-2 rounded-lg">
            <Shield size={24} className="text-white" />
          </div>
          {!collapsed && (
            <h1 className="text-xl font-semibold text-slate-900 ml-3">
              Memora
            </h1>
          )}
        </div>
      ...
      <div
        className={`middle flex flex-col gap-1 ${collapsed ? "px-2" : "px-4"} py-6`}
      >
